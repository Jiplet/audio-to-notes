...
description: Transcribe voice memos and audio recordings using OpenAI Whisper, then structure and write to an Obsidian vault folder. Two modes — (1) Single-file: user drops an audio path or says "transcribe this", "voice memo". (2) Scan inbox: user says "process voice memos", "any new memos", "scan voice folder" — processes every audio file in the inbox folder and moves each to processed/ after success. Also trigger when the user asks to capture a meeting or conversation from audio.
---

# Voice Memo Transcriber

Transcribes audio using OpenAI Whisper, structures it into meeting notes, and writes one markdown file per memo to an Obsidian vault.
...
Voice files arrive here automatically (e.g. via Samsung → Autosync → Google Drive → Drive Desktop on Mac, or any sync flow you set up). After a successful wiki write, files move to the `processed/` subfolder so the inbox always shows pending work.

## Output target

Voice notes are written as markdown files to:

`<OBSIDIAN_VAULT_PATH>/<VOICE_NOTES_FOLDER>/`

> Replace both placeholders before using.
...
```bash
python3 .claude/skills/utility-transcribe/scripts/transcribe.py "<audio_file_path>"
```

If `openai` or `requests` aren't installed, install first:
```bash
...
Produce this structure. Be concise.
...
- Recommended approach (phased if appropriate)
...
`<OBSIDIAN_VAULT_PATH>/<VOICE_NOTES_FOLDER>/<YYYY-MM-DD> - <slug>.md`
...
area: <one of: <AREA_ENUM>>
people: [Name1, Name2]   # named participants — omit field entirely if none
tags: [voice-note, <2-4 topical tags in kebab-case>]
---
```

> Replace `<AREA_ENUM>` with your own taxonomy. Example: `ProjectAlpha | ProjectBeta | Personal | General`.
...
```bash
mv "<source_path>" "<INBOX_PATH>/processed/"
```
...
For single-file mode where the source isn't in the inbox (e.g. an ad-hoc path the user gave), skip the move — just tell the user the path of the new vault note.

## API Key

The script loads `OPENAI_API_KEY` from `.env` in this skill directory:
```
.claude/skills/utility-transcribe/.env
```

If the key is missing, prompt: "Add your OpenAI API key to `.claude/skills/utility-transcribe/.env` — see `.env.example` for format."

## Supported formats
m4a, mp3, mp4, mpeg, mpga, wav, webm, ogg (OpenAI Whisper limits: 25MB per file).

## Large files (auto-handled)
The script handles oversize files automatically:
1. **>25 MB** → compress to 32 kbps mono AAC via `ffmpeg`
...
## Samsung 3GPP handling

Samsung's stock voice recorder writes 3GPP audio with a `.m4a` extension. Whisper rejects this. The script detects the `ftyp` brand directly and re-encodes via `ffmpeg` before sending. No manual intervention needed.
